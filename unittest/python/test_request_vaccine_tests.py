...
class TestRequestVacID(unittest.TestCase):
    ...
    def test_parametrized_valid_request_vaccination(self):
        ...
        for patient_id, name_surname, registration_type, phone_number, age, expected_result, comment in param_list_ok:
    ...
    def test_parametrized_not_valid_request_vaccination(self):
        ...
        my_request = VaccineManager()
        for patient_id, name_surname, registration_type, phone_number, age, expected_result, comment in param_list_nok:
            with self.subTest(test=comment):
                with self.assertRaises(VaccineManagementException) as context_manager:
                    my_request.request_vaccination_id(patient_id, name_surname, registration_type, phone_number, age)
                self.assertEqual(context_manager.exception.message, expected_result)
                self.assertIsNone(file_store.find_item(patient_id, "_VaccinePatientRegister__patient_id"))
    ...
    def test__duplicate_valid_request_vaccination(self):
        ...
        file_store.delete_json_file()
        my_request = VaccineManager()
        value = my_request.request_vaccination_id("a729d963-e0dd-47d0-8bc6-b6c595ad0098",# noqa
